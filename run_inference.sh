...
for bestn in 64; do
    ...
    # echo $!
    python scripts/run_bon_scoring.py --model="$pref_model" --ref_model="$ref_model" --num_threads 2 --batch_size=4 --pref_sets "$output_dir/${filename}_rewards.jsonl" > logs/$filename.log 2>&1 &
    echo $!
done
