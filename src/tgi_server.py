...
class TGI:
    def __init__(self, port=8000):
        ...
        """
        self.port = port
        self.client = InferenceClient(model=f"http://0.0.0.0:{port}")
                ...
                for item_idx, data in enumerate(chunk_results):
                    ...
                    results.append(
                        dict(
                            ...
                            generated_text=data["generated_text"].strip(),
                            # stop_reason="",
...
if __name__ == "__main__":
    vllm_server = TGI(port=8080)
    results = vllm_server.make_vllm_request(["i like beijing "])
